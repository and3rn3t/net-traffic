...
- [x] **Live activity ticker** — compact real-time feed of notable events (new device, threat, anomaly, alert trigger) fed by existing WS message types. `useActivityFeed.ts` subscribes to `device_update`/`threat_update`/`alert_triggered` via the shared `useWebSocketSubscription`, keeping a 50-event ring buffer. `ActivityTicker.tsx` widget (opt-in, `defaultEnabled: false` in the widget catalog) renders it with severity badges and relative timestamps.

### Dashboard widgets

Composable widgets for the customizable dashboard (each also viable standalone):

- [x] **Network health score** — single 0–100 gauge blending threat count, anomaly rate, retransmission rate, and latency vs. baseline. `useNetworkHealthScore.ts` blends the server-computed connection-quality score (40% weight, `getConnectionQualitySummary`) with a threat-severity penalty (35%, from the `activeThreats` prop) and a device-anomaly penalty (25%, summed `device.behavioral.anomalyCount` across the `devices` prop) into one 0–100 score with excellent/good/fair/poor severity banding. `NetworkHealthScoreWidget.tsx` shows the score with a hover tooltip breaking down each factor's contribution.
...
- [x] **This week vs last week** — overlay chart comparing the current 7-day bandwidth window against the prior one, with a total-bytes % delta callout. `useWeekComparison.ts` fetches a single 336h/60min `getBandwidthTimeline` call and splits it into two aligned 168-point series (bucketed by hours-ago-from-now); `WeekComparisonWidget.tsx` renders both as a recharts line overlay plus an up/down delta badge.
- [ ] **Device presence heatmap** — online/offline grid by device × hour ("who's home"), from existing device last-seen data.
- [x] **DNS insights** — top queried domains, NXDOMAIN/failure rate, unusual TLD flags, from the shipped DNS decoding. New backend endpoint `GET /api/stats/dns` (`aggregate_dns_stats` SQL aggregate in `storage/flows.py`, blended in `AnalyticsService.get_dns_stats`) returns query volume, response-code breakdown, top queried domains with per-domain failure counts, and unusual (uncommon) TLDs. `useDnsInsights.ts` polls it every 60s; `DnsInsightsWidget.tsx` shows the failure-rate badge, a top-domains mini table, and unusual-TLD badges.
