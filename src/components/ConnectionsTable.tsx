...
// Memoized so unrelated re-renders of the parent (e.g. filter panel state)
          ...
          <div className="space-y-2">
            {displayFlows.length === 0 ? (
              ...
              displayFlows.map(flow => (
                <FlowRow key={flow.id} flow={flow} onClick={handleFlowClick} />
              ))
            )}
          </div>
        </ScrollArea>
