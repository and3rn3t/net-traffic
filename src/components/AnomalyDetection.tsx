...
export function AnomalyDetection({ flows, devices }: AnomalyDetectionProps) {
  const detectAnomalies = (): Anomaly[] => {
    ...
    const avgTotalBytes = deviceTraffic.reduce((sum, d) => sum + d.totalBytes, 0) / (devices.length || 1)
    ...
    const repeatedConnections = flows.reduce((acc, flow) => {
  ...
  const overallScore = anomalies.length === 0 ? 0 : 
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>
              AI-powered behavioral analysis of network patterns
            </CardDescription>
          </div>
