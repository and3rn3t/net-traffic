...
export function SecurityPosture({ flows, devices, threats }: SecurityPostureProps) {
  const calculateSecurityScore = (): { overall: number; metrics: SecurityMetric[] } => {
    ...
    const highThreatFlows = flows.filter(f => f.threatLevel === 'high' || f.threatLevel === 'critical').length
    ...
    const avgDeviceThreatScore = devices.reduce((sum, d) => sum + d.threatScore, 0) / (devices.length || 1)
    ...
    const concentrationRisk = uniqueDestinations < 20 ? 100 : Math.max(0, 100 - ((uniqueDestinations - 20) * 2))
    metrics.push({
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>
              Overall network security health assessment
            </CardDescription>
          </div>
          <div className="text-right">
            <div className="text-3xl font-bold">
              <span className={getScoreColor(overall)}>
                {getScoreGrade(overall)}
              </span>
            </div>
      ...
      <CardContent className="space-y-6">
        <div>
          <div className="flex items-center justify-between text-sm mb-2">
            ...
            <span className={`font-medium ${getScoreColor(overall)}`}>
              {overall.toFixed(0)}%
            </span>
          </div>
          <Progress 
            ...
            {overall >= 80 ? (
              <>
                ...
                <div>
                  ...
                  <div className="text-xs text-muted-foreground">
                    Your network demonstrates good security practices. Continue monitoring for emerging threats.
                  </div>
                </div>
              </>
            ) : overall >= 60 ? (
              <>
                ...
                <div>
                  ...
                  <div className="text-xs text-muted-foreground">
                    Some security metrics need attention. Review failed checks and take corrective action.
                  </div>
                </div>
              </>
            ) : (
              <>
                ...
                <div>
                  ...
                  <div className="text-xs text-muted-foreground">
                    Immediate action required. Multiple security metrics are below acceptable thresholds.
                  </div>
                </div>
              </>
            )}
