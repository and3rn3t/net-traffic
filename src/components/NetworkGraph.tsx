...
export function NetworkGraph({ flows, devices }: NetworkGraphProps) {
  ...
  useEffect(() => {
    ...
    const links: GraphLink[] = flows.slice(0, 20).map(f => ({
      source: f.deviceId,
    ...
    const link = g.append('g')
      ...
      .attr('stroke', d => {
        switch (d.threatLevel) {
          case 'critical':
          case 'high': return 'oklch(0.55 0.20 25)'
          case 'medium': return 'oklch(0.70 0.15 75)'
          case 'low': return 'oklch(0.65 0.15 200)'
          default: return 'oklch(0.45 0.08 250)'
        }
      })
      ...
      .attr('fill', d => {
        if (d.type === 'device') {
          switch (d.threatLevel) {
            case 'high': return 'oklch(0.55 0.20 25)'
            case 'medium': return 'oklch(0.70 0.15 75)'
            default: return 'oklch(0.65 0.15 200)'
          }
        }
