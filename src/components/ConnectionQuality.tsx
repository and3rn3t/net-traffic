...
export function ConnectionQuality({ flows }: ConnectionQualityProps) {
  const metrics = useMemo(() => {
    ...
    const avgPacketSize = flows.reduce((sum, f) => {
    ...
    const retransmissionRate = flows.reduce((sum, f) => {
      const expectedPackets = f.packetsIn + f.packetsOut
      const actualEfficiency = expectedPackets > 0 ? (f.bytesIn + f.bytesOut) / expectedPackets : 1
      return sum + (actualEfficiency < 1000 ? 1 : 0)
    }, 0) / (flows.length || 1) * 100

    const protocolEfficiency = flows.reduce((acc, f) => {
      if (!acc[f.protocol]) {
    ...
    const avgBandwidthUtilization = flows.reduce((sum, f) => {
  ...
  const getQualityBadge = (score: number) => {
    if (score >= 80) return { label: 'Excellent', variant: 'default' as const, icon: <CheckCircle size={16} className="text-success" /> }
    if (score >= 60) return { label: 'Good', variant: 'secondary' as const, icon: <CheckCircle size={16} className="text-success" /> }
    if (score >= 40) return { label: 'Fair', variant: 'outline' as const, icon: <Warning size={16} className="text-warning" /> }
    return { label: 'Poor', variant: 'destructive' as const, icon: <XCircle size={16} className="text-destructive" /> }
  }
      ...
      <CardContent className="space-y-6">
          ...
          <div className="w-32 h-32">
            <svg viewBox="0 0 100 100" className="transform -rotate-90">
              ...
              <circle
                ...
                r="40"
                fill="none"
                stroke={metrics.qualityScore >= 80 ? 'hsl(var(--success))' : 
                       metrics.qualityScore >= 60 ? 'hsl(var(--primary))' : 
                       metrics.qualityScore >= 40 ? 'hsl(var(--warning))' : 'hsl(var(--destructive))'}
                strokeWidth="8"
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xl font-bold">{(metrics.avgBandwidthUtilization / 1024).toFixed(0)} KB/s</p>
          </div>
        </div>

        <div className="space-y-3">
          <p className="text-sm font-semibold">Protocol Efficiency</p>
          {Object.entries(metrics.protocolEfficiency).slice(0, 5).map(([protocol, stats]) => {
            ...
            return (
              <div key={protocol} className="space-y-1">
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="text-muted-foreground">{efficiency.toFixed(0)}% efficient</span>
                </div>
